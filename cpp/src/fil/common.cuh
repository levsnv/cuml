...
struct shmem_size_params {
  ...
  /// n_items is the most items per block that fit into shared memory
  int n_items = 0;
  /// shm_sz is the associated shared memory footprint
  int shm_sz = INT_MAX;

  __host__ __device__ size_t sdata_stride() {
    return num_cols + 1 - num_cols % 2;  // pad to odd
  }
  __host__ __device__ size_t cols_shmem_size() {
    return cols_in_shmem ? sizeof(float) * sdata_stride() * n_items : 0;
  }
